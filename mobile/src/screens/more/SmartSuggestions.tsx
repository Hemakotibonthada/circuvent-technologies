...
export default function SmartSuggestions({ onBack, onOpenAutomate, onOpenDevices, onOpenSettings }: { onBack: () => void; onOpenAutomate: () => void; onOpenDevices: () => void; onOpenSettings: () => void }) {
  ...
  const state = useAsync<number>(async () => (await unwrap<{ scenes: unknown[] }>(api.scenes(), "your scenes")).scenes.length, []); const scenes = state.data ?? 0;
  ...
  return <Screen><ScrollView contentContainerStyle={{ padding: 16, paddingTop: 56, paddingBottom: 90 }}><View style={{ flexDirection: "row", alignItems: "center", gap: 12, marginBottom: 16 }}><IconButton glyph="‹" onPress={onBack} /><Title>Suggestions</Title></View><StaleNotice error={state.error} onRetry={state.reload} /><SectionLabel>Actionable</SectionLabel>{items.map((x) => <Card key={x.t} style={{ marginBottom: 10 }}><Text style={{ color: c.text, fontWeight: "900" }}>{x.t}</Text><Text style={{ color: c.textDim, marginTop: 6 }}>{x.b}</Text><PrimaryButton label="Take action" onPress={x.go} style={{ marginTop: 10 }} /></Card>)}</ScrollView></Screen>;
}
