      ...
      {variance && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-3 text-center">
            <p className="text-xl font-bold text-blue-600 dark:text-blue-400">₹{(variance.totalBudgeted / 100000).toFixed(1)}L</p>
            <p className="text-xs text-slate-500">Total Budgeted</p>
          </div>
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-3 text-center">
            <p className="text-xl font-bold text-amber-600 dark:text-amber-400">₹{(variance.totalActual / 100000).toFixed(1)}L</p>
            <p className="text-xs text-slate-500">Total Spent</p>
          </div>
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-3 text-center">
            <p className={`text-xl font-bold ${variance.totalVariance >= 0 ? "text-emerald-600 dark:text-emerald-400" : "text-red-600 dark:text-red-400"}`}>₹{(Math.abs(variance.totalVariance) / 100000).toFixed(1)}L</p>
            <p className="text-xs text-slate-500">{variance.totalVariance >= 0 ? "Under Budget" : "Over Budget"}</p>
          </div>
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-3 text-center">
            <p className="text-xl font-bold text-red-600 dark:text-red-400">{variance.overBudgetCount}</p>
            ...
            <tbody>
              {budgets.map((b: any) => (
                <tr key={b.id} className="border-b border-slate-200/50 dark:border-slate-800/50 hover:bg-slate-800/30">
                  <td className="px-4 py-3 text-sm text-brand-600 dark:text-brand-400 font-mono">{b.accountCode}</td>
                  ...
                  <td className="px-4 py-3 text-sm text-right font-mono text-amber-600 dark:text-amber-400">₹{Number(b.spent).toLocaleString("en-IN")}</td>
                  <td className={`px-4 py-3 text-sm text-right font-mono ${b.remaining >= 0 ? "text-emerald-600 dark:text-emerald-400" : "text-red-600 dark:text-red-400"}`}>₹{Number(b.remaining).toLocaleString("en-IN")}</td>
