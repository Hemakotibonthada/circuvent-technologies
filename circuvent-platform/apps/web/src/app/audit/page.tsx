...
export default function AuditPage() {
  ...
  const columns = [
    { id: "createdAt", header: "Timestamp", render: (l: AuditLog) => <span className="font-mono text-xs">{formatDateTime(l.createdAt)}</span> }, { key: "action", header: "Action", render: (l: AuditLog) => <Badge color={actionColors[l.action] || "slate"}>{l.action}</Badge> }, { key: "entity", header: "Entity", render: (l: AuditLog) => <span className="text-slate-900 dark:text-white">{l.entity}</span> }, { key: "entityId", header: "Entity ID", render: (l: AuditLog) => l.entityId ? <span className="font-mono text-xs text-slate-500">{l.entityId.slice(0, 8)}...</span> : "—" }, { key: "user", header: "User", render: (l: AuditLog) => l.user ? (
      <div>
    ...
    ) : <span className="text-slate-500">System</span> }, { key: "ipAddress", header: "IP", render: (l: AuditLog) => <span className="font-mono text-xs text-slate-500">{l.ipAddress || "—"}</span> },
  ];
