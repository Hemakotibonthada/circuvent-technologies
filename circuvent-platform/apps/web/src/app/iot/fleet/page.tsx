...
export default function IoTFleetPage() {
  ...
  const tabs = [
    ...
    { id: "alerts", label: "Active Alerts", count: health?.criticalAlerts },
    ...
    <div className="space-y-6">
      ...
      {health?.averageMetrics && (
        <div className="grid grid-cols-3 gap-4">
          <Card className="text-center">
            ...
            <p className={`text-3xl font-bold ${health.averageMetrics.avgCpu > 80 ? "text-red-400" : "text-slate-900 dark:text-white"}`}>
          ...
          <Card className="text-center">
            ...
            <p className={`text-3xl font-bold ${health.averageMetrics.avgMemory > 85 ? "text-red-400" : "text-slate-900 dark:text-white"}`}>
          ...
          <Card className="text-center">
            ...
            <p className={`text-3xl font-bold ${health.averageMetrics.avgTemperature > 70 ? "text-amber-400" : "text-slate-900 dark:text-white"}`}>
      ...
      {activeTab === "alerts" && health?.devicesNeedingAttention && (
        <Card padding={false}>
          <DataTable
            columns={[
              { key: "deviceCode", header: "Device", render: (a: any) => <span className="font-mono text-xs text-brand-400">{a.deviceCode}</span> },
              { key: "deviceName", header: "Name" },
              { key: "alertType", header: "Alert", render: (a: any) => <Badge color="red">{a.alertType}</Badge> },
              { key: "severity", header: "Severity", render: (a: any) => <Badge color={a.severity === "CRITICAL" ? "red" : "amber"}>{a.severity}</Badge> },
              { key: "message", header: "Message", render: (a: any) => <span className="text-xs">{a.message}</span> },
              { key: "since", header: "Since", render: (a: any) => timeAgo(a.since) },
            ]}
      ...
      {activeTab === "commands" && (
        <Card>
          ...
          <p className="text-sm text-slate-400 text-center py-8">
            Use the <a href="/iot/health" className="text-brand-400 hover:underline">Health Monitor</a> to send commands to specific devices.
          </p>
        </Card>
      )}
    </div>
