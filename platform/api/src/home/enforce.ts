...
export function refuse(req: AuthedRequest, capability: Parameters<typeof can>[1]): string | null {
  const home = req.home;
  if (!home || home.actorId === home.homeId) return null;
  return can(home.role, capability) ? null : refusalFor(home.role, capability);
}
 ...
 * there is one obvious place to look. The implementation lives in `actor.ts`,
 * which imports no database — see the note there.
 */
export { actorId, asActor } from "./actor";
