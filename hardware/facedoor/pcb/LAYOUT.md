# Circuvent FaceDoor - generated PCB layout
...
| | |
| --- | --- |
| Board file | `cv-door.kicad_pcb` |
| Custom design rules | `cv-door.kicad_dru` |
| Board size | **99.5 x 83.0 mm** (doc target 70 x 55 mm) |
| Layers | 2 (F.Cu / B.Cu), FR4 1.6 mm, 1 oz Cu |
| Footprints placed | 42 of 42 BOM positions |
| Nets | 58 total, 29 multi-pad (routable) |
| Pads bound to nets | 166 of 166 |
| Net classes | Default=22, MAINS=5, POWER=3 |
| Routing | autorouted, 664 track/via segments |
| DRC errors / unconnected | **2 / 8** |
...
`cv-door.kicad_dru` adds the rules a net class cannot express: reinforced
...
Unused BOM positions with no documented connection (fit as DNP or delete from the BOM): `R11`, `R12`, `R13`, `R14`

## Residual unconnected items - hand-finish list
The autorouter left 8 connection(s) open. Each one is a real missing copper
...
| 2 | `AC_L_FUSED` | MAINS | Track on F.Cu, length 5.0000 mm | PTH pad 1 of K1 |
| 3 | `AC_L_FUSED` | MAINS | PTH pad 1 of K1 | PTH pad 1 of PS1 |
| 4 | `AC_N` | MAINS | Track on F.Cu, length 40.2573 mm | PTH pad 3 of J2 |
| 5 | `AC_N` | MAINS | PTH pad 3 of J2 | PTH pad 2 of PS1 |
| 6 | `GND` | POWER | PTH pad 3 of PS1 | Zone on B.Cu, priority 0 |
| 7 | `GND` | POWER | Zone on B.Cu, priority 0 | Pad 2 of C8 on B.Cu |
| 8 | `LOCK_RELAY_SW` | MAINS | PTH pad 2 of J2 | PTH pad 3 of K1 |
...
| --- | --- | --- | --- |
| `J2` | term3 | AC_N, LOCK_RELAY_SW | +5V |
| `K1` | relay | AC_L_FUSED, LOCK_RELAY_SW, N$K1.4 | +5V, LOCK_RELAY_COIL |
...
- [x] Copper routing (autorouted, 664 track/via segments)
...
```
cd hardware
"<KiCad>/bin/python.exe" gen-pcb.py facedoor --fab
```
