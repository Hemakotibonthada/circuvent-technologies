# Circuvent ANPR Gate Camera - generated PCB layout
...
| | |
| --- | --- |
| Board file | `cv-anpr.kicad_pcb` |
| Custom design rules | `cv-anpr.kicad_dru` |
| Board size | **57.8 x 53.7 mm** (doc target 55 x 50 mm) |
| Layers | 2 (F.Cu / B.Cu), FR4 1.6 mm, 1 oz Cu |
| Footprints placed | 21 of 21 BOM positions |
| Nets | 23 total, 19 multi-pad (routable) |
| Pads bound to nets | 70 of 70 |
| Net classes | Default=16, POWER=3 |
| Routing | not run |
...
`cv-anpr.kicad_dru` adds the rules a net class cannot express: reinforced
...
```
cd hardware
"<KiCad>/bin/python.exe" gen-pcb.py anpr-camera --fab
```
