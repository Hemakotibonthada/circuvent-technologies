# Circuvent RFID Gate - generated PCB layout
...
| | |
| --- | --- |
| Board file | `cv-gate.kicad_pcb` |
| Custom design rules | `cv-gate.kicad_dru` |
| Board size | **103.1 x 88.0 mm** (doc target 75 x 60 mm) |
| Layers | 2 (F.Cu / B.Cu), FR4 1.6 mm, 1 oz Cu |
| Footprints placed | 44 of 44 BOM positions |
| Nets | 63 total, 27 multi-pad (routable) |
| Pads bound to nets | 172 of 172 |
| Net classes | Default=19, MAINS=6, POWER=3 |
| Routing | autorouted, 625 track/via segments |
...
`cv-gate.kicad_dru` adds the rules a net class cannot express: reinforced
...
Unused BOM positions with no documented connection (fit as DNP or delete from the BOM): `R10`, `R11`, `R12`, `R13`, `R14`
...
| 2 | `AC_L_FUSED` | MAINS | PTH pad 1 of PS1 | Track on B.Cu, length 6.1407 mm |
| 3 | `OPEN_RELAY_SW` | MAINS | PTH pad 1 of J2 | PTH pad 3 of K1 |
| 4 | `AC_N` | MAINS | Track on F.Cu, length 10.9500 mm | PTH pad 2 of PS1 |
| 5 | `CLOSE_RELAY_SW` | MAINS | PTH pad 2 of J2 | PTH pad 3 of K2 |
...
| --- | --- | --- | --- |
| `K1` | relay | AC_L_FUSED, N$K1.4, OPEN_RELAY_SW | +5V, OPEN_RELAY_COIL |
| `K2` | relay | AC_L_FUSED, CLOSE_RELAY_SW, N$K2.4 | +5V, CLOSE_RELAY_COIL |
...
- [x] Copper routing (autorouted, 625 track/via segments)
...
```
cd hardware
"<KiCad>/bin/python.exe" gen-pcb.py rfid-gate --fab
```
