# Circuvent Adapter 12V 1A - generated PCB layout
...
| | |
| --- | --- |
| Board file | `cv-ad12.kicad_pcb` |
| Custom design rules | `cv-ad12.kicad_dru` |
| Board size | **82.4 x 54.8 mm** (doc target 50 x 28 mm) |
| Layers | 2 (F.Cu / B.Cu), FR4 1.6 mm, 1 oz Cu |
| Footprints placed | 29 of 29 BOM positions |
| Nets | 27 total, 17 multi-pad (routable) |
| Pads bound to nets | 75 of 75 |
| Net classes | Default=4, HVDC=9, MAINS=3, POWER=2 |
| Routing | not run |
...
`cv-ad12.kicad_dru` adds the rules a net class cannot express: reinforced
...
- Top and bottom edge bands are reserved for mounting holes, fiducials,
  test points (+5V) and silkscreen legends.
...
```
cd hardware
"<KiCad>/bin/python.exe" gen-pcb.py psu-adapter-12v --fab
```
