# Circuvent Sentinel Safety Panel - generated PCB layout
...
| | |
| --- | --- |
| Board file | `cv-sent.kicad_pcb` |
| Custom design rules | `cv-sent.kicad_dru` |
| Board size | **100.1 x 122.1 mm** (doc target 90 x 70 mm) |
| Layers | 2 (F.Cu / B.Cu), FR4 1.6 mm, 1 oz Cu |
| Footprints placed | 60 of 60 BOM positions |
| Nets | 73 total, 42 multi-pad (routable) |
| Pads bound to nets | 213 of 213 |
| Net classes | Default=32, MAINS=8, POWER=3 |
| Routing | autorouted, 964 track/via segments |
| DRC errors / unconnected | **0 / 11** |
...
`cv-sent.kicad_dru` adds the rules a net class cannot express: reinforced
...
Unused BOM positions with no documented connection (fit as DNP or delete from the BOM): `R12`, `R13`, `R14`, `R15`, `R16`

## Residual unconnected items - hand-finish list
The autorouter left 11 connection(s) open. Each one is a real missing copper
connection and has to be drawn by hand (or designed out) before fabrication:

| # | Net | Class | From | To |
| --- | --- | --- | --- | --- |
| 1 | `AC_L_FUSED` | MAINS | Track on F.Cu, length 2.9713 mm | PTH pad 1 of PS1 |
| 2 | `RELAY4_SW` | MAINS | PTH pad 2 of J3 | PTH pad 3 of K4 |
| 3 | `GND` | POWER | Zone on B.Cu, priority 0 | Zone on B.Cu, priority 0 |
| 4 | `GND` | POWER | Zone on F.Cu, priority 0 | Zone on F.Cu, priority 0 |
| 5 | `GND` | POWER | Zone on B.Cu, priority 0 | Zone on F.Cu, priority 0 |
| 6 | `GND` | POWER | Zone on B.Cu, priority 0 | Pad 2 of C6 on B.Cu |
| 7 | `RELAY3_SW` | MAINS | PTH pad 1 of J3 | PTH pad 3 of K3 |
| 8 | `AC_N` | MAINS | PTH pad 2 of PS1 | Track on F.Cu, length 17.9000 mm |
| 9 | `UART_RX` | Default | Pad 34 of U1 on F.Cu | PTH pad 2 of JP |
| 10 | `RELAY2_SW` | MAINS | PTH pad 2 of J2 | PTH pad 3 of K2 |
| 11 | `RELAY1_SW` | MAINS | PTH pad 1 of J2 | PTH pad 3 of K1 |
...
| `K4` | relay | AC_L_FUSED, N$K4.4, RELAY4_SW | +5V, RELAY4_COIL |
...
- [x] Copper routing (autorouted, 964 track/via segments)
...
```
cd hardware
"<KiCad>/bin/python.exe" gen-pcb.py sentinel --fab
```
