# Circuvent Guardian SOS - generated PCB layout
...
`cv-sos.kicad_dru` adds the rules a net class cannot express: reinforced
...
- GND pour on both layers over the low-voltage area, tied together with GND vias
  ...
  topped up again afterwards; board outline is rounded with 1 mounting hole(s).
...
```
cd hardware
"<KiCad>/bin/python.exe" gen-pcb.py guardian --fab
```
